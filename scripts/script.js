...
function calcFinalSizeSrink() {
 ...
 for (let index = 0; index < idElement; index++) {
  ...
  inputParameters[`element-${index}`].nks =
   (((inputParameters[`element-${index}`]["flex-basis"] || 0) +
    inputParameters.parameters.gsfs *
     (inputParameters[`element-${index}`]["flex-shrink"] || 0)) *
    (inputParameters[`element-${index}`]["flex-shrink"] || 0)) /
  ...
  inputParameters[`element-${index}`].irs =
   (inputParameters[`element-${index}`]["flex-basis"] || 0) -
   inputParameters.parameters.gsfs *
    (inputParameters[`element-${index}`]["flex-shrink"] || 0) -
   Math.abs(
    inputParameters[`element-${index}`].nks * inputParameters.parameters.op
   );
 }
}
...
function calcFinalSizeGrow() {
 ...
 for (let index = 0; index < idElement; index++) {
  ...
  inputParameters.parameters.dsm +=
   inputParameters[`element-${index}`]["flex-grow"] || 0;
 }
 ...
 for (let k = 0; k < idElement; k++) {
  ...
  inputParameters[`element-${k}`].irr =
   (inputParameters[`element-${k}`]["flex-basis"] || 0) -
   inputParameters.parameters.gsfg *
    (inputParameters[`element-${k}`]["flex-grow"] || 0) +
   inputParameters.parameters.dsm *
    (inputParameters[`element-${k}`]["flex-grow"] || 0);
 }
...
function showIrsIrr() {
 ...
 for (let index = 0; index < idElement; index++) {
  if (
   isNaN(inputParameters[`element-${index}`]?.nks) ||
   inputParameters[`element-${index}`]?.nks < 0
  ) {
   ...
   nks[index].textContent =
    Math.floor(inputParameters[`element-${index}`]?.nks * 10) / 10;
  }

  if (
   isNaN(inputParameters[`element-${index}`]?.irs) ||
   inputParameters[`element-${index}`]?.irs > pageWidth ||
   inputParameters[`element-${index}`]?.irs < 0
  ) {
  ...
  if (
   isNaN(inputParameters[`element-${index}`]?.irr) ||
   inputParameters[`element-${index}`]?.irr > pageWidth ||
   inputParameters[`element-${index}`]?.irr < 0
  ) {
   ...
   irr[index].textContent = `${Math.round(
    inputParameters[`element-${index}`]?.irr
   )}px`;
  }
 }
}
...
function showPreview() {
 ...
 for (let index = 0; index < idElement; index++) {
  ...
  flexElement.style.cssText = `align-self: ${
   inputParameters[`element-${index}`]["align-self"]
  };
...
function showCssCode() {
 ...
 for (let j = 0; j < idElement; j++) {
  if (
   inputParameters[`element-${j}`]["flex-grow"] === 0 &&
  ...
  for (let key in inputParameters[`element-${j}`]) {
   if (
    ...
    inputParameters[`element-${j}`][key] === "auto"
   )
    continue;

   cssStyles.innerHTML += `  ${key}: ${
    inputParameters[`element-${j}`][key]
   }${hasPx(key)};<br/>`;
  }
